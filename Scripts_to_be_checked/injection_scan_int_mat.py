            ...
            for mycalib in range(calibstart, calibstop, calibstep):
                for key in i2csocket.yamlConfig.keys():
                    if key.find('roc_s')==0:
                        ...
                        nestedConf[key]['sc']['ReferenceVoltage']['all']['Calib'] = mycalib
                        print("Calib: ", mycalib)
                i2csocket.configure(yamlNode=nestedConf.to_dict())
...
def sampling_scan(i2csocket,daqsocket, clisocket, basedir,device_name, injectionConfig,suffix=""):
    ...
    odir = "%s/%s/injection_scan/TB3_2_TOA230_220_BX22_PHASE14/"%( os.path.realpath(basedir), device_name ) # a comlete path is needed
...
if __name__ == "__main__":
    ...
    injectionConfig = {
        ...
        'calib' : 400,
        'injectedChannels' : [a, b, 36+a, 36+b]
    }
