...
class vrefinv_scan_analyzer(analyzer):
    ...
    def makePlots(self):
        ...
        sel_data = self.data[['chip','channel','channeltype','adc_median','adc_iqr','Inv_vref','half']].copy()
        for chip in range(nchip):
            ...
            ax.scatter(chan_data['Inv_vref'], chan_data['adc_median'], c=inv, cmap=cmap)
            
            chan_data = data[ (data['channeltype']==100) & (data['half']==0) ].copy()
            u, inv = np.unique(chan_data.channel.values, return_inverse=True)
            ax.scatter(chan_data['Inv_vref'], chan_data['adc_median'], c=inv, cmap=cmcmap)
            ...
            chan_data = data[ (data['channeltype']<=1) & (data['half']==0) ].copy()
            u, inv = np.unique(chan_data.channel.values, return_inverse=True)
            ax.scatter(chan_data['Inv_vref'], chan_data['adc_iqr'], c=inv, cmap=cmap)
            
            chan_data = data[ (data['channeltype']==100) & (data['half']==0) ].copy()
            u, inv = np.unique(chan_data.channel.values, return_inverse=True)
            ax.scatter(chan_data['Inv_vref'], chan_data['adc_iqr'], c=inv, cmap=cmcmap)
            ...
            medians = data[ data['channeltype']==0 ].groupby(['half','Inv_vref']).median().reset_index()
            fig, axes = plt.subplots(1,2,figsize=(16,9),sharey=True)
            for half in range(2):
                ax=axes[half]
                half_data = medians[ medians['half']==half ]
                ax.scatter(half_data['Inv_vref'], half_data['adc_median'], color='blue')
            
                xs, alpha, beta = self.fit( half_data, 'Inv_vref', 'adc_median' )
    ...
    def determine_bestVrefInv(self):
        nchip = len( self.data.groupby('chip').nunique() )
        data = self.data[['chip','channel','channeltype','adc_median','adc_iqr','Inv_vref','half']].copy()
        yaml_dict={}
        ...
        for chip in range(nchip):
            if chip<len(rockeys):
                ...
                medians = data[ (data['chip']==chip) & (data['channeltype']==0) ].groupby(['half','Inv_vref']).median().reset_index()
                for half in range(2):
                    half_data = medians[ medians['half']==half ]
                    xs, alpha, beta = self.fit( half_data, 'Inv_vref', 'adc_median' )
                    yaml_dict[chip_key_name]['sc']['ReferenceVoltage'][half] = { 'Inv_vref' : int( (target_ped-beta)/alpha ) }
            else :
